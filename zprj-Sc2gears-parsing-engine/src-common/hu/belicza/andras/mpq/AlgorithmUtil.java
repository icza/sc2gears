 ...
 * 
 * <p>These include:
 * <ul><li>Decryption utility for MPQ encrypted data
 * <li>Hash calculation for a string</ul>
 * (They are based on code from StormLib.)</p>
 * 
...
class AlgorithmUtil {
	...
	static {
		...
		for ( index1 = 0; index1 < 0x100; index1++ ) {
			for ( index2 = index1, i = 0; i < 5; i++, index2 += 0x100 ) {
				...
				seed  = ( ( seed  * 125 + 3 ) & 0xFFFFFFFFL ) % 0x2AAAABL;
	 ...
	 * 
	 * <p>The decryption will wrap the data with a byte buffer, and decrypt inside that (it shares the original byte buffer).
	 * 
	...
	public static ByteBuffer decryptData( final ByteBuffer buffer, final int size, final int key ) {
		long seed1 = key & 0xFFFFFFFFL;
		long seed2 = 0xEEEEEEEEL;
		int ch;
		
		for ( int i = 0; i < size; i += 4 ) {
			seed2 = ( seed2 + CRYPT_TABLE[ 0x400 + (int) ( seed1 & 0xFF ) ] ) & 0xFFFFFFFFL;
			ch = buffer.getInt( i ) ^ (int) ( seed1 + seed2 );
			
			seed1  = ( ( ( ( ~seed1 & 0xFFFFFFFFL ) << 0x15 ) + 0x11111111L ) & 0xFFFFFFFFL ) | ( seed1 >> 0x0B );
			seed2 = ( ( ch & 0xFFFFFFFFL ) + seed2 + ( seed2 << 5 ) + 3 ) & 0xFFFFFFFFL;
	 ...
	 */
	public static int hashString( final String string, final MpqHashType mpqHash ) {
		...
		for ( int i = 0; i < length; i++ ) {
			...
			seed2 = ( ch + seed1 + seed2 + ( seed2 << 5 ) + 3 ) & 0xFFFFFFFFL; // ch is small so it does not need to be masked (it's a char)
		}
