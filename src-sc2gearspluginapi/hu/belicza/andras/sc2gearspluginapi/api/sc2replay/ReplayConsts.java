...
public class ReplayConsts {
	...
	public static enum Gateway {
		// Season start reference: Tuesday 0:00 AM, PDT => Monday 16:00, GMT
		/** America.        */  // Season start: Tuesday 5 AM, PDT = Tuesday 12 PM, GMT
		AMERICA( "US", "sc2.gateway.america", 12L*60*60*1000, "PST8PDT", "http://usb.depot.battle.net:1119/", "http://us.battle.net/",
		...
		ASIA( "KR", "sc2.gateway.asia", 44L*60*60*1000, "ROK", "http://krb.depot.battle.net:1119/", "http://kr.battle.net/",
		...
		CHINA( "CN", "sc2.gateway.china", 21L*60*60*1000, "CTT", "http://cnb.depot.battle.net:1119/", "http://www.battlenet.com.cn/",
		...
		EUROPE( "EU", "sc2.gateway.europe", 28L*60*60*1000, "CET", "http://eub.depot.battle.net:1119/", "http://eu.battle.net/",
		...
		SOUTHEAST_ASIA( "SG", "sc2.gateway.southeastAsia", -3L*60*60*1000, "SST", "http://sg.depot.battle.net:1119/", "http://sea.battle.net/",
				BnetLanguage.ENGLISH, EnumSet.of( BnetLanguage.ENGLISH ) ),
		/** Public test.    */
		PUBLIC_TEST( "XX", "sc2.gateway.publicTest", 12L*60*60*1000, "GMT", "http://xx.depot.battle.net:1119/", "http://us.battle.net/",
				BnetLanguage.ENGLISH, EnumSet.of( BnetLanguage.ENGLISH ) ),
		/** Unknown.        */
		UNKNOWN( "", "general.unknown", 0L, "GMT", "" , "" , null, EnumSet.noneOf( BnetLanguage.class ) );
		
		/** The binary value (stored in the replay file) of the gateway. */
		public final String              binaryValue;
		/** Cache of the string value.                                   */
		public final String              stringValue;
		/** Season start time offset in milliseconds compared to the reference date specified in the {@link LadderSeason#startDate}. */
		public final long                seasonStartTimeOffset;
		/**
		 * Time zone offset of the gateway's local time.
		 * @since "3.0.1"
		 */
		 ...
		 * @param timeZoneId                   time zone id of the gateway's local time
		 ...
		 */
		private Gateway( final String binaryValue, final String textKey, final long seasonStartTimeOffset, final String timeZoneId, final String depotServerUrl, final String battleNetUrl, final BnetLanguage defaultLanguage, final Set< BnetLanguage > availableLanguageSet ) {
			this.binaryValue           = binaryValue;
			stringValue                = Language.getText( textKey );
			this.seasonStartTimeOffset = seasonStartTimeOffset;
			timeZoneOffset             = TimeZone.getTimeZone( timeZoneId ).getOffset( System.currentTimeMillis() );
