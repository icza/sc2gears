...
public class ApiCallStatInfo implements IsSerializable {
	...
	public void integrateApiCallStat( final ApiCallStatInfo info ) {
		calls            += info.calls;
		usedOps          += info.usedOps;
		execTime         += info.execTime;
		deniedCalls      += info.deniedCalls;
		errors           += info.errors;
		infoCalls        += info.infoCalls;
		mapInfoCalls     += info.mapInfoCalls;
		parseRepCalls    += info.parseRepCalls;
		profInfoCalls    += info.profInfoCalls;
		infoExecTime     += info.infoExecTime;
		mapInfoExecTime  += info.mapInfoExecTime;
		parseRepExecTime += info.parseRepExecTime;
		profInfoExecTime += info.profInfoExecTime;
	}
