...
/**
 * Search field that filters by a research that was issued.
 * 
 * @author Andras Belicza
 */
public class ResearchSearchField extends ComboBoxSearchField {
	
	/** Vector of researches to be added to the combo box. */
	private static final Vector< Object > RESEARCH_VECTOR = createDataVector( EnumSet.allOf( Research.class ) );
	
	/**
	 * Creates a new ResearchSearchField.
	 */
	public ResearchSearchField() {
		super( Id.RESEARCH, RESEARCH_VECTOR );
	}
		...
		return comboBox.getSelectedIndex() == 0 ? null : new ComboBoxReplayFilter( this ) {
			...
			public boolean customAccept( final File file, final Replay replay ) {
				...
				for ( int i = actions.length - 1; i >= 0; i-- )
					if ( ( action = actions[ i ] ).type == ActionType.RESEARCH && ( (ResearchAction) action ).research == selected )
						if ( --minOccurrence_ == 0 )
							return true;
				return false;
			}
	...
	public Icon getIcon( final Object value ) {
		return value instanceof Research ? Icons.getResearchIcon( (Research) value, IconSize.MEDIUM ) : IconHandler.NULL.get( IconSize.MEDIUM );
	}
