...
public class ReplayUtils {
	...
	/**
	 * Parses a TGA image from its binary data.<br>
	 * The image must be uncompressed true color image (type=2).
	 * This is the format of the map previews stored in SC2 map files.
	 * 
	 * @param data data to parse the TGA image from
	 * @return an ImageIcon containing the parsed image
	 */
	public static ImageIcon parseTgaImage( final byte[] data ) {
		...
		int pos = 18;
		int rgb, x1 = Integer.MAX_VALUE, y1 = Integer.MAX_VALUE, x2 = Integer.MIN_VALUE, y2 = Integer.MIN_VALUE;
		for ( int y = 0; y < height; y++ )
			for ( int x = 0; x < width; x++ ) {
				rgb = ( data[ pos+2 ] & 0xff ) << 16 | ( data[ pos+1 ] & 0xff ) << 8 | ( data[ pos ] & 0xff );
				if ( rgb != 0 ) {
		...
		return new ImageIcon( bufferedImage.getSubimage( x1, y1, x2 - x1 + 1, y2 - y1 + 1 ) );
	}
	...
	private static AbilityCodes loadAbilityCodes( final String fileName ) {
		...
		try ( final BufferedReader input = new BufferedReader( new InputStreamReader( ReplayUtils.class.getResourceAsStream( fileName ) ) ) ) {
